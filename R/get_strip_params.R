...
.get_strip_params <- function(airport) {
  ...
  for (pista_id in names(airport$pistas)) {
    ...
    if (is.null(code_number) || is.null(approach_type)) {
      stop(sprintf("Faltam dados em pista %s: code=%s, approach_type=%s",
                   pista_id,
                   code_number %||% "NULL",
                   approach_type %||% "NULL"))
    }
    ...
    if (is.null(param_encontrado)) {
      stop(sprintf("RBAC-154: combinação inválida → Code %d + %s (pista %s)",
...
create_strip_result <- function(pista_id, pista_data, params) {
  list(
    ...
    descricao = sprintf("Code %d%s | %s → ±%d m lateral | +%d m longitudinal",
